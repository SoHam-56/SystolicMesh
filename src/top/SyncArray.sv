...
// Drop-in for SystolicArray: same write, start, re-arm, completion and read ports.
module SyncArray #(
    parameter int N           = 4,
  ...
  initial begin
    if ((AD % WEST_WORDS) != 0 || (AD % NORTH_WORDS) != 0)
      $error("SyncArray: write widths %0d/%0d must divide %0d", WEST_WORDS, NORTH_WORDS, AD);
  end
...
`ifndef SYNTHESIS
  a_start_idle: assert property (@(posedge clk_i) disable iff (!rstn_i) start_matrix_mult_i |-> !feeding)
    else $error("SyncArray: started while still feeding the previous matmul");
  a_loaded: assert property (@(posedge clk_i) disable iff (!rstn_i) start_matrix_mult_i |-> (wa == AD && wb == AD))
    else $error("SyncArray: started before both operands were fully written");
`endif
