...
// Output-stationary PE for SyncArray: takes one product every cycle and passes its operands on a cycle later.
...
`ifndef SYNTHESIS
  a_no_late_product: assert property (@(posedge clk_i) disable iff (!rstn_i) prod_v |-> (st == P_ACC))
    else $error("SyncPE: a product arrived after the partial sums were being combined");
  a_slot_free: assert property (@(posedge clk_i) disable iff (!rstn_i) (st == P_ACC && prod_v) |-> (inflight < 4'(S)))
    else $error("SyncPE: more adds in flight than partial sums");
`endif
